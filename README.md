...
Cloudflare 变量只用于服务端读取 Analytics GraphQL API。推荐生产环境使用仅包含 Analytics Read 权限的账户或用户 API Token；后端通过 `Authorization: Bearer` 发送该 Token。旧的 `CLOUDFLARE_EMAIL` + `CLOUDFLARE_API_KEY` Global API Key 配置仍兼容，但不推荐继续使用。不要在仓库中保存任何密钥。
...
Cloudflare variables are server-only and power the Analytics GraphQL API. In production, prefer an account-owned or user API Token with only Analytics Read permission; the backend sends it with `Authorization: Bearer`. The legacy `CLOUDFLARE_EMAIL` + `CLOUDFLARE_API_KEY` Global API Key configuration remains supported but is not recommended. Never commit secrets to the repository.
