...
CLOUDFLARE_API_KEY=cloudflare-global-key-or-scoped-token
CLOUDFLARE_ZONE_ID=cloudflare-zone-id
```

Cloudflare 变量只用于服务端读取 Zone Analytics。推荐生产环境使用仅包含 Zone Analytics Read / Zone Read 权限的 Cloudflare API Token，避免在仓库中保存任何密钥。
...
`traffic` 统计来自 Cloudflare Zone Analytics。`24h`、`7d`、`30d` 返回访问者和请求数；`total` 用服务端保存的起始时间累计请求数，页面只展示请求总数。
...
Cloudflare variables are server-only and power Zone Analytics traffic stats. In production, prefer a scoped Cloudflare API Token with Zone Analytics Read / Zone Read permissions and never commit secrets to the repository.
...
The `traffic` endpoint uses Cloudflare Zone Analytics. `24h`, `7d`, and `30d` return visitors and requests; `total` accumulates request count from the server-side baseline and is displayed as requests only.
