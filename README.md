...
当前版本使用 TanStack Start、React 19、TanStack Router、TanStack Query、Base UI、Tailwind CSS 4 和 shadcn/ui 风格组件构建 Web 应用。API 服务使用 Go、pgx、Redis、PostgreSQL 和 miekg/dns。
...
- API 在线状态和延迟显示、Cloudflare Zone Analytics 请求统计、favicon、SEO 元信息、站点地图友好路由和 Umami 统计脚本
...
- pnpm
- Go 1.26+
- PostgreSQL 18.4（使用 Docker Compose 时自动提供）
...
> PostgreSQL 18 的官方 Docker 镜像使用版本化的数据目录。Compose 已将持久卷挂载到
> `/var/lib/postgresql`，实际 `PGDATA` 为 `/var/lib/postgresql/18/docker`。如果已有
> PostgreSQL 16 数据卷，请先使用 `pg_dump`/`pg_dumpall` 导出，再导入 PostgreSQL 18，或使用
> `pg_upgrade`；主版本之间不能直接复用旧数据目录。升级时不要执行
> `docker compose down -v`，否则会删除已有数据卷。
...
CLOUDFLARE_API_TOKEN=cloudflare-account-or-user-api-token
CLOUDFLARE_ZONE_ID=cloudflare-zone-id
```

Cloudflare 变量只用于 API 服务读取 Analytics GraphQL API。推荐生产环境使用仅包含 Analytics Read 权限的账户或用户 API Token；API 服务通过 `Authorization: Bearer` 发送该 Token。旧的 `CLOUDFLARE_EMAIL` + `CLOUDFLARE_API_KEY` Global API Key 配置仍兼容，但不推荐继续使用。不要在仓库中保存任何密钥。
...
curl "https://api.dns.nf/v1/dns/stats/traffic?range=24h"
curl "https://api.dns.nf/v1/dns/stats/traffic?range=7d"
curl "https://api.dns.nf/v1/dns/stats/traffic?range=30d"
curl "https://api.dns.nf/v1/dns/stats/traffic?range=total"
```

`traffic` 统计来自 Cloudflare Zone Analytics。`24h`、`7d`、`30d` 返回访问者和请求数；`total` 从服务端保存的起始日期累计，并在 Cloudflare 日统计保留期内重新校准历史回填数据，超过保留期后使用持久化基线继续累计。
...
- API health latency indicator, Cloudflare Zone Analytics traffic counters, favicon set, sitemap-friendly routes, and Umami analytics scripts
...
- pnpm
- Go 1.26+
- PostgreSQL 18.4 (provided automatically by Docker Compose)
...
> The official PostgreSQL 18 image uses a version-specific data directory. Compose mounts the
> persistent volume at `/var/lib/postgresql`, with `PGDATA` set to
> `/var/lib/postgresql/18/docker`. If you already have a PostgreSQL 16 volume, export it with
> `pg_dump`/`pg_dumpall` and restore it into PostgreSQL 18, or use `pg_upgrade`; major versions
> cannot directly share a data directory. Do not run `docker compose down -v` during the upgrade,
> because it deletes the existing data volume.
...
Cloudflare variables are used only by the API service to access the Analytics GraphQL API. In production, prefer an account-owned or user API Token with only Analytics Read permission; the API service sends it with `Authorization: Bearer`. The legacy `CLOUDFLARE_EMAIL` + `CLOUDFLARE_API_KEY` Global API Key configuration remains supported but is not recommended. Never commit secrets to the repository.
...
The `traffic` endpoint uses Cloudflare Zone Analytics. `24h`, `7d`, and `30d` return visitors and requests; `total` accumulates from the server-side start date, reconciles late Cloudflare backfills while daily data remains available, and continues from the persisted baseline after the retention window.
