import { d as defineEventHandler, u as useRuntimeConfig, f as getHeader, c as createError, g as getQuery } from '../../nitro/nitro.mjs';
import { reverse, resolveCname } from 'node:dns/promises';
import 'node:http';
import 'node:https';
import 'node:events';
import 'node:buffer';
...
const RDNS_COOLDOWN_MS = 3e4;
const lastScanAtByClient = /* @__PURE__ */ new Map();
setInterval(() => {
...
const lookupCNAME = async (hosts, timeoutMs) => {
  const out = [];
...
const lookupPTR = async (ip, timeoutMs) => {
  try {
    ...
    return {
      ptr: [],
      ok: false,
      error: (err == null ? void 0 : err.message) || "lookup failed"
    };
  }
};
const createRateLimiter = (ratePerSec) => {
  const intervalMs = Math.max(1, Math.floor(1e3 / Math.max(1, ratePerSec)));
...
const buildTargets = (target) => {
  ...
  const networkMask = parsed.mask === 0 ? 0 : -1 << 32 - parsed.mask >>> 0;
...
const getClientIp = (event) => {
  ...
  if (xff) {
    const first = (_a = xff.split(",")[0]) == null ? void 0 : _a.trim();
...
const rdns_get = defineEventHandler(async (event) => {
  var _a;
  const config = useRuntimeConfig(event);
  const internalToken = String(config.rdnsInternalToken || "").trim();
  if (internalToken) {
    const reqToken = String(getHeader(event, "x-rdns-token") || "").trim();
    if (!reqToken || reqToken !== internalToken) {
      throw createError({ statusCode: 403, statusMessage: "forbidden: internal rdns api" });
    }
  }
  ...
  if (delta < RDNS_COOLDOWN_MS) {
    const waitSeconds = Math.ceil((RDNS_COOLDOWN_MS - delta) / 1e3);
    throw createError({
      statusCode: 429,
  ...
  const filteredResults = keyword ? results.filter((row) => {
    const pool = matchTarget === "ptr" ? row.ptr : matchTarget === "cname" ? row.cname : [...row.ptr, ...row.cname];
  ...
  if (ptrItems.length > 0) {
    ...
    const apiBase = String(
      config2.apiInternalBase || ((_a = config2.public) == null ? void 0 : _a.apiBase) || "http://localhost:8080"
    ).replace(/\/$/, "");
    const internalToken2 = String(config2.internalToken || "").trim();
    const headers = { "Content-Type": "application/json" };
    if (internalToken2) headers["X-Internal-Token"] = internalToken2;
...
export { rdns_get as default };
//# sourceMappingURL=rdns.get.mjs.map
